class Sortable {
  ...
  // 监听下拉、上拉点击事件
  recheck(){
    let pullDown = document.getElementsByClassName("span_xiala")
    let pullUp = document.getElementsByClassName("span_shangla")
    Array.from(pullDown).forEach(v => {
      v.addEventListener("click", () => {
      clearTimeout(pullDown.sortableDResize)
      pullDown.sortableDResize = setTimeout(() =>{
        this._setBlocWidth(()=>{
        this.orderelements()
      })
      }, 50)
    ...
    Array.from(pullUp).forEach(v => {
      v.addEventListener("click", () => {
      clearTimeout(pullUp.sortableUResize)
      pullUp.sortableUResize = setTimeout(() =>{
        this._setBlocWidth(()=>{
        this.orderelements()
      })
      }, 50)
